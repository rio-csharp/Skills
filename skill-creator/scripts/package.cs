...
static int Run(string[] args)
{
    ...
    {
        return Fail("package requires a skill path and optional output directory");
    }

    try
    {
        var archivePath = PackageSkill(args[0], args.Length == 2 ? args[1] : null);
        Console.WriteLine($"Packaged skill: {archivePath}");
        return 0;
    }
    catch (Exception ex)
    {
...
static string PackageSkill(string skillPath, string? outputDirectory)
{
    var source = Path.GetFullPath(skillPath);
    var validation = ValidateSkill(source);
    if (!validation.Ok)
    {
        throw new InvalidOperationException("validation failed: " + string.Join("; ", validation.Errors));
    }

    var destinationDirectory = string.IsNullOrWhiteSpace(outputDirectory)
        ? Directory.GetCurrentDirectory()
        : Path.GetFullPath(outputDirectory);

    Directory.CreateDirectory(destinationDirectory);

    var archivePath = Path.Combine(destinationDirectory, $"{Path.GetFileName(source)}.skill");
    ...
    foreach (var filePath in Directory.EnumerateFiles(source, "*", SearchOption.AllDirectories).OrderBy(x => x))
    {
        ...
        var archiveName = $"{Path.GetFileName(source)}/{relative.Replace('\\', '/')}";
        archive.CreateEntryFromFile(filePath, archiveName, CompressionLevel.SmallestSize);
    }
...
{
    var excludeDirectories = new HashSet<string>(StringComparer.Ordinal)
    {
        ".git", ".hg", ".svn", "__pycache__", "node_modules", ".venv", "venv", "dist", "build", "evals", "tmp", "temp"
    };
    var excludeFiles = new HashSet<string>(StringComparer.Ordinal) { ".DS_Store", "Thumbs.db" };
    var excludeSuffixes = new[] { ".pyc", ".pyo", ".log", ".tmp", ".skill" };
    ...
    return parts.Any(excludeDirectories.Contains);
}
...
static int Fail(string message)
{
    ...
    Console.Error.WriteLine("  dotnet run --file <this-skill>/scripts/package.cs -- <path-to-skill-folder> [output-directory]");
    return 1;
}
